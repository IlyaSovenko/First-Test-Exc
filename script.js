...
function onMouseApp(out) {
    if(out){
        ...
        document.querySelector('.app').style.top = window.innerHeight*0.3 - 56 + 'px';
        document.querySelector('.app').style.left = window.innerWidth*0.4 - 56 + 'px';
        document.querySelector('.btn-add-row').style.marginLeft = '62px';
    }
}
...
document.querySelector('.btn-add-row').onclick = () => {
    ...
    let nextNumOfRow = 1 +  Number(rows[rows.length-1].className.split('-')[1]);
...
{
    let numOfCol = document.querySelector('.col:last-child').className.split('-')[1];
...
document.querySelector('.btn-del-row').onclick = () => {
    let numOfRow = document.querySelector('.row:last-child').className.split('-')[1];
